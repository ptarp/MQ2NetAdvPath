	...
	if( FollowPath.size() ) {
		...
		if( GetDistance(GetCharInfo()->pSpawn->X,GetCharInfo()->pSpawn->Y,CurList->X,CurList->Y) > DISTANCE_BETWEEN_LOG+DistanceMod ) {
			...
			switch(CurList->Animation) {
				case 19: case 20: 
					if(GetCharInfo()->standstate==STANDSTATE_STAND) ExecuteCmd(FindMappableCommand("jump"),1,0);
					break;
				case 22: case 24:
					if(GetCharInfo()->standstate==STANDSTATE_STAND) ExecuteCmd(FindMappableCommand("duck"),1,0); 
					break;
				default: 
					if(GetCharInfo()->standstate==STANDSTATE_DUCK) ExecuteCmd(FindMappableCommand("duck"),1,0); 
					break;
			}
...
VOID StuckCheck(bool reset) {
	...
	if( FollowPath.size() && UseStuckLogic && !reset && StatusState == STATUS_ON && ( FollowState == FOLLOW_FOLLOWING ) ) {
		...
		if ( FindSpeed(GetCharInfo()->pSpawn) && StatusState == STATUS_ON && !GetCharInfo()->Stunned && GetCharInfo()->pSpawn->SpeedMultiplier != -10000 ) {
				...
				if ( Jump && GetCharInfo()->pSpawn->mPlayerPhysicsClient.Levitate == 2 && !GetCharInfo()->pSpawn->UnderWater && (!pulseMoved || (pulseMoved < 0.1 && pulseMoved < expectedPulseMoved)) && JumpTimer + 1 < now) {
					Jump = false;
					JumpTimer = time(NULL);
					ExecuteCmd(FindMappableCommand("JUMP"),1,0);
					ExecuteCmd(FindMappableCommand("JUMP"),0,0);
				}
...
class MQ2NetAdvPathType : public MQ2Type {
	...
	public:
		...
		MQ2NetAdvPathType():MQ2Type("NetAdvPath"), Temps{ 0 } {
			TypeMember(Active);
			TypeMember(State);
		...
		virtual bool MQ2NetAdvPathType::GetMember(MQVarPtr VarPtr, const char* Member, char* Index, MQTypeVar& Dest) override {
			std::list<Position>::iterator CurList=FollowPath.begin();
			std::list<Position>::iterator EndList=FollowPath.end();
			int i = 1;
			float TheLength = 0;
		...
		/*virtual bool FromData(MQ2VARPTR &VarPtr, MQ2TYPEVAR &Source) override {
			return false;
		} 
